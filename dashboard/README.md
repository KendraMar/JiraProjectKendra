...
- **Comments render rich formatting** (headings, bold, links, lists, etc.) via dangerouslySetInnerHTML inside PatternFly Content component
...
- **Fixed description corruption on save** — the app was stripping HTML tags from the ADF-converted description, concatenating all block elements (headings, paragraphs, bullets) into a single paragraph. Now tracks whether the user actually edited the description and only sends it if modified.
- **Fixed activity type prefix corrupting descriptions** — the `handleSave` function was prepending `[Activity Type]` to the HTML description on every save, triggering the destructive HTML-to-plaintext conversion. Reverted to only prepend on new issue titles.
- **Fixed comments displaying raw HTML tags** — comments were going through `adfToHtml` but rendered with `{c.body}` (plain text). Switched to `dangerouslySetInnerHTML` inside PatternFly Content component.
- **Fixed PatternFly CSS stripping description formatting** — headings rendered as plain bold text, lists lost bullets. Added `!important` CSS overrides for heading sizes (h1–h4), list styles (`list-style: revert`), list item display, bold, italic, code, links, and `<p>` inside `<li>` margin removal.
