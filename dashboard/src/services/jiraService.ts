...
const STATIC_ISSUES: JiraIssue[] = [
  ...
  { key: "CPUX-6111", summary: "Scheduler share with accelerator program", issueType: "Story", status: "In Progress", statusCategory: "In Progress", priority: "Medium", activityType: "Explore", epicKey: "CPUX-5566", epicName: "Scheduler MVP", storyPoints: null, sprintName: "HCC Sprint 3 2026", sprintState: "active" as JiraIssue["sprintState"], sprintStartDate: "April 6", sprintEndDate: "April 17", assigneeName: "Yichen Yu", assigneeAvatar: "https://avatar-management--avatars.us-west-2.prod.public.atl-paas.net/633b2eae2eaaa5dcfa164a94/afa1fff8-d49e-42d6-8c06-5f36066339b4/32", reporterName: "Yichen Yu", description: "### What\n\nShare the scheduler mocks with attendees in the accelerator program\n\n### Definition of Done\n\nThis issue should be Closed once the following criteria are met.\n\n* The primary deliverable or artifact is attached or linked to this issue.\n* The primary deliverable of this issue has been shared for review with the UXD team members first, followed by the PM and Engineering teams (if appropriate).\n* For UI-specific designs (aka things you make in Figma), the primary deliverable has been through a critique review session with at least one other designer.\n* Follow-up tasks identified during the review have been created in the parent epic.\n* \\[Include any other criteria that need to be met before closing this issue\\]\n\n", dueDate: "", comments: [] },
  ...
  { key: "CPUX-6173", summary: "[Explore] Simplify marketplace subs to HCC connection experience", issueType: "Story", status: "In Progress", statusCategory: "In Progress", priority: "Medium", activityType: "Consult", epicKey: "CPUX-5956", epicName: "[UXD EPIC] UXD-Driven in Q1", storyPoints: null, sprintName: "HCC Sprint 3 2026", sprintState: "active" as JiraIssue["sprintState"], sprintStartDate: "April 6", sprintEndDate: "April 17", assigneeName: "Kendra Marchant", assigneeAvatar: "https://avatar-management--avatars.us-west-2.prod.public.atl-paas.net/557058:78b37446-6818-4ae8-8fb9-02b4cfbecdc7/40177a56-70a9-478c-97ad-0d4bd7e9aa77/32", reporterName: "Kendra Marchant", description: "### What\n\nSupport team in understanding requirements and flows, reviewing mockups and providing feedback, creating presentations and presenting to stakeholders. \n\n### Definition of Done\n\nThis issue should be Closed once the following criteria are met.\n\n* The primary deliverable or artifact is attached or linked to this issue.\n* The primary deliverable of this issue has been shared for review with the UXD team members first, followed by the PM and Engineering teams (if appropriate).\n* For UI-specific designs (aka things you make in Figma), the primary deliverable has been through a critique review session with at least one other designer.\n* Follow-up tasks identified during the review have been created in the parent epic.\n* \\[Include any other criteria that need to be met before closing this issue\\]\n\n", dueDate: "", comments: [{ author: "Kendra Marchant", authorAvatar: "https://avatar-management--avatars.us-west-2.prod.public.atl-paas.net/557058:78b37446-6818-4ae8-8fb9-02b4cfbecdc7/40177a56-70a9-478c-97ad-0d4bd7e9aa77/32", body: "<strong>Status update</strong><br/><strong>Done:</strong><ul><li>Met with Yichen to discuss next steps for the marketplace subs simplification effort</li></ul><strong>Next:</strong><ul><li>Reach out to TAM Gordon Keegan about distributing the MCP and script to other TAMs for testing</li></ul>", created: "Apr 8, 2026" }] },
  { key: "CPUX-6172", summary: "move to correct epic: Custom (AI gen) dashboard widgets", issueType: "Story", status: "In Progress", statusCategory: "In Progress", priority: "Medium", activityType: "Consult", epicKey: "CPUX-5956", epicName: "[UXD EPIC] UXD-Driven in Q1", storyPoints: null, sprintName: "HCC Sprint 3 2026", sprintState: "active" as JiraIssue["sprintState"], sprintStartDate: "April 6", sprintEndDate: "April 17", assigneeName: "Kendra Marchant", assigneeAvatar: "https://avatar-management--avatars.us-west-2.prod.public.atl-paas.net/557058:78b37446-6818-4ae8-8fb9-02b4cfbecdc7/40177a56-70a9-478c-97ad-0d4bd7e9aa77/32", reporterName: "Kendra Marchant", description: "### What\n\nSupport team in understanding requirements and flows, reviewing mockups and providing feedback, creating presentations and presenting to stakeholders. \n\n### Definition of Done\n\nThis issue should be Closed once the following criteria are met.\n\n* The primary deliverable or artifact is attached or linked to this issue.\n* The primary deliverable of this issue has been shared for review with the UXD team members first, followed by the PM and Engineering teams (if appropriate).\n* For UI-specific designs (aka things you make in Figma), the primary deliverable has been through a critique review session with at least one other designer.\n* Follow-up tasks identified during the review have been created in the parent epic.\n* \\[Include any other criteria that need to be met before closing this issue\\]\n\n", dueDate: "", comments: [] },
...
function markdownToHtml(md: string): string {
  ...
  const flushBullets = () => {
    ...
    parts.push("<ul>" + bulletBuf.map((b) => `<li>${inlineFmt(b)}</li>`).join("") + "</ul>");
    bulletBuf = [];
  };
  const flushOrdered = () => {
    if (!orderedBuf.length) return;
    parts.push("<ol>" + orderedBuf.map((b) => `<li>${inlineFmt(b)}</li>`).join("") + "</ol>");
    orderedBuf = [];
  };
  ...
  for (const line of lines) {
    ...
    if (headingMatch) {
      ...
      parts.push(`<h${lvl}>${inlineFmt(headingMatch[2])}</h${lvl}>`);
      continue;
    }
    if (/^[-*]\s+/.test(trimmed)) {
      flushOrdered();
...
function repairFlattenedAdf(adf: unknown): unknown {
  if (!adf || typeof adf !== "object") return adf;
  const doc = adf as AdfNode;
  if (doc.type !== "doc" || !doc.content) return adf;

  if (doc.content.length > 2) return adf;

  const allText = doc.content
    .flatMap((block) => (block.content ?? []).map((c) => c.text ?? ""))
    .join("");
  if (allText.length < 80) return adf;

  const HEADINGS = ["What", "Definition of Done", "Problem Statement", "problem statement", "Acceptance Criteria", "Description"];
  const splitRe = new RegExp(
    `(${HEADINGS.map((h) => h.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join("|")})`,
    "g"
  );

  const hasEmbeddedHeadings = HEADINGS.some((h) => {
    const idx = allText.indexOf(h);
    if (idx < 0) return false;
    const before = idx > 0 ? allText[idx - 1] : " ";
    const after = allText[idx + h.length] ?? " ";
    const boundaryBefore = !/[a-z]/i.test(before) || /[.!?\s]/.test(before) || idx === 0;
    const isJammed = /[A-Z]/.test(after) || /\u00a0/.test(after);
    return boundaryBefore && (isJammed || /[A-Z]/.test(before));
  });

  if (!hasEmbeddedHeadings) return adf;

  const parts = allText.split(splitRe).filter(Boolean);
  ...
  for (const part of parts) {
    ...
    if (HEADINGS.includes(cleaned)) {
      newContent.push({ type: "heading", attrs: { level: 3 }, content: [{ type: "text", text: cleaned }] });
      continue;
    }

    const sentences = cleaned.split(/\.(?=[A-Z])/).map((s) => s.trim()).filter(Boolean);
    if (sentences.length > 2) {
      newContent.push({
        type: "paragraph",
        content: [{ type: "text", text: sentences[0] + (sentences[0].endsWith(".") ? "" : ".") }],
      });
      newContent.push({
        type: "bulletList",
        content: sentences.slice(1).map((s) => ({
          type: "listItem",
          content: [{ type: "paragraph", content: [{ type: "text", text: s + (s.endsWith(".") ? "" : ".") }] }],
        })),
      });
    } else {
      for (const s of sentences) {
        newContent.push({ type: "paragraph", content: [{ type: "text", text: s + (s.endsWith(".") ? "" : ".") }] });
      }
    }
  }

  if (newContent.length <= 1) return adf;
  return { type: "doc", version: 1, content: newContent };
}

function extractText(adf: unknown): string {
  return adfToHtml(repairFlattenedAdf(adf));
}
...
function descriptionToAdf(desc: string): Record<string, unknown> {
  const isHtml = /<(?:h[1-6]|p|div|ul|ol|li|br|strong|em|table|blockquote|pre|a |img )[\s>/]/.test(desc);
...
export function groupBySprint(issues: JiraIssue[]): SprintGroup[] {
  ...
  for (const issue of issues) {
    const sprintEntries = (issue.allSprints && issue.allSprints.length > 0)
      ? issue.allSprints
      : issue.sprintName
        ? [{ name: issue.sprintName, state: issue.sprintState, startDate: issue.sprintStartDate, endDate: issue.sprintEndDate }]
        : [];
...
export async function startSprint(
  ...
  const sprintInfo = HCC_SPRINT_DATES[sprintName];
  const now = new Date();
  const year = now.getFullYear();
  const startDate = sprintInfo
    ? new Date(`${sprintInfo.startDate} ${year}`).toISOString()
    : now.toISOString();
  const endDate = sprintInfo
    ? new Date(`${sprintInfo.endDate} ${year}`).toISOString()
    : new Date(now.getTime() + 14 * 24 * 60 * 60 * 1000).toISOString();

  const res = await jiraFetch(`${apiBase()}/rest/agile/1.0/sprint/${sprintId}`, {
    method: "PUT",
    headers: authHeaders(),
  ...
  if (!res.ok) {
    ...
    throw new Error(`Failed to start sprint (${res.status}): ${text}`);
  }
}

const HCC_SPRINT_DATES: Record<string, { startDate: string; endDate: string }> = {
  "HCC Sprint 1 2026": { startDate: "March 9", endDate: "March 20" },
  "HCC Sprint 2 2026": { startDate: "March 23", endDate: "April 3" },
  "HCC Sprint 3 2026": { startDate: "April 8", endDate: "April 24" },
  "HCC Sprint 4 2026": { startDate: "April 20", endDate: "May 1" },
};
